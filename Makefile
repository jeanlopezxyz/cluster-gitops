...
.PHONY: help bootstrap status clean helm-repo package-charts
...
helm-repo: ## Deploy Helm repository on bastion server
	@echo "📦 Deploying Helm Chart Repository..."
	cd ansible && ansible-playbook -i inventories/production/hosts.yaml playbooks/deploy-helm-repo.yaml -v

helm-repo-local: ## Deploy Helm repository locally (for testing)
	@echo "📦 Deploying Helm Chart Repository locally..."
	cd ansible && ansible-playbook -i inventories/production/hosts.yaml playbooks/deploy-helm-repo.yaml -v -e "bastion_ip=localhost"
...
package-charts: ## Package all Helm charts
	...
	@helm repo index charts/ --url http://$${HELM_REPO_URL:-localhost:8080}/charts
